...
namespace UI
{
    ...
    public class ChatHub : Hub
    {
        ...
        public void Connect(string UserName, int UserID, string UserImage) {
            var id = Context.ConnectionId;
            if (ConnectedUsers.Count(x => x.ConnectionId == id) == 0)
            {
                ConnectedUsers.Add(new UserDetail { ConnectionId = id, UserName = UserName + "-" + UserID, UserID = UserID , UserImage = UserImage});
            }
